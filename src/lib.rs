...
#[wasm_bindgen(js_name = "denoRequireFetch")]
pub fn deno_require_fetch(s: &str) -> Result<JsValue, JsValue> {
    let resp = request::get(s, &mut Default::default());

    let json = resp.body.to_json().to_string();
    ...
    try_with_handler(
        ...
        |_handler| {
            c.run(|| {
                ...
                let s = c
                    .print(&program, None, None, true, opts.codegen_target().unwrap_or(EsVersion::Es2020), opts.source_maps
                        .clone()
                        .unwrap_or(SourceMapsConfig::Bool(false)), &Default::default(), None, bool::from(opts.config.minify), None, false, false)
